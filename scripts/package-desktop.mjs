...
await fs.mkdir(path.join(root, "tmp", "desktop-release"), { recursive: true });
const release = await fs.realpath(path.join(root, "tmp", "desktop-release"));
...
for (const name of ["main.mjs", "protocol.mjs"]) await fs.copyFile(path.join(root, "desktop", name), path.join(stage, name));
await fs.writeFile(path.join(stage, "package.json"), JSON.stringify({
  ...
  version: "1.0.25", author: "Yautja La Longue Chasse - projet de fan", private: true, type: "module", main: "main.mjs",
  description: "Jeu de fan non commercial - edition PC hors ligne V25",
}, null, 2));
...
await fs.writeFile(path.join(release, "manifest-v25.json"), JSON.stringify({
  version: "1.0.25", electron: pkg.devDependencies.electron, platform: "win32-x64",
  createdAt: new Date().toISOString(), signed: false, networkRequired: false, hashes,
}, null, 2));
console.log(JSON.stringify({ directory, manifest: path.join(release, "manifest-v25.json"), hashes }, null, 2));
