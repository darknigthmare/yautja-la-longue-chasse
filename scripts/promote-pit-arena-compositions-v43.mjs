...
import {parseArenaNumbers,assertHistoricalArenasUnchanged} from './lib/pit-screen-arena-v43.mjs';
...
assert(/^docs\/v43-[a-z0-9/-]+[.]json$/.test(visualPath??'')&&!visualPath.includes('..'),'An actual committed composition visual review is required');
...
if(mode==='integrate'){
 ...
 assert(/^docs\/v43-[a-z0-9/-]+[.]json$/.test(appProofPath??'')&&!appProofPath.includes('..'));
...
for(const number of ids){
 ...
 const evidence=`docs/v43-${stage.catalogueId}-renderer-qa.json`;
