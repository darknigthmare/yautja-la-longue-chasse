...
const proofPath=process.argv[3]??`work/v34/fullapp-qa/${arenaId}/browser-qa.json`;
assert(new RegExp("^work/v(?:34|35)/[a-z0-9/.-]+[.]json$").test(proofPath)&&!proofPath.includes(".."));
...
const stage=manifest.stages.find(s=>s.legacyRuntimeArenaId===arenaId||s.runtimeExtension?.arenaId===arenaId);
assert(stage?.runtimeEnabled);
const evidence=`docs/v34-${arenaId}-fullapp-qa.json`;
await fs.writeFile(evidence,proof,{flag:"wx"});
if(stage.runtimeExtension)stage.runtimeExtension.applicationEvidence=evidence;
