...
function render() {
  ...
  if (view.value === 'frames' && sequence.length && current.status !== 'rejected') {
    ...
    const anchorX = current.previewAnchor?.[0] ?? 600, anchorY = current.previewAnchor?.[1] ?? 730;
    // Fit the entire pivot-aligned sequence, including effects that extend to only one side.
    const extent = (measure: (item: ReviewFrame) => number) => Math.max(1, ...sequence.map(measure));
    const scale = Math.min(2, 1 / size, (anchorX - 24) / extent(item => item.pivot[0]),
      (1200 - anchorX - 24) / extent(item => item.rect[2] - item.pivot[0]),
      (anchorY - 24) / extent(item => item.pivot[1]), (800 - anchorY - 24) / extent(item => item.rect[3] - item.pivot[1]));
    const [x, y, w, h] = frame.rect;
    context.drawImage(prepared, x, y, w, h, anchorX - frame.pivot[0] * scale, anchorY - frame.pivot[1] * scale, w * scale, h * scale);
...
async function select() {
  ...
  canvas.dataset.assetId = '';canvas.dataset.loaded = 'false';element('name').textContent = 'Aucune image sélectionnée';element('status').textContent = '';element('summary').textContent = '';element('notes').replaceChildren();element<HTMLAnchorElement>('source').removeAttribute('href');buttons();render();
  if (!selected) return;
...
function filter() {
  const query = element<HTMLInputElement>('search').value.toLocaleLowerCase('fr').trim();
  ...
  const filtered = entries.filter(entry => (category.value === 'all' || entry.category === category.value)
    && (status === 'all' || entry.status === status || (status === 'reviewed' && entry.status === 'integrated'))
    && (!query || (entry.name + ' ' + entry.id).toLocaleLowerCase('fr').includes(query)));
  element('results').textContent = filtered.length + ' image(s) trouvée(s)';
...
element('search').addEventListener('input', filter);element('status-filter').addEventListener('change', filter);
