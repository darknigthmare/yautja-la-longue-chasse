...
const expectedClips = ['idle', 'high-guard', 'pit.stand.light.startup', 'pit.stand.light.active', 'pit.stand.light.recovery'];
assert.deepEqual(validatedClips.map(({ clip }) => `${clip.id}:${clip.facing}`).sort(),
  expectedClips.flatMap(id => ['right', 'left'].map(facing => `${id}:${facing}`)).sort(),
  'Only the reviewed masked Ahab idle, high guard and complete light attack may be certified');
      ...
      if (observed && source) {
        ...
        if (args.length === 9) {
          ...
          for (const mapping of mappings.filter(item => source.endsWith(item.src) && item.rect.every((value, index) => value === args[index + 1]))) {
            evidence.draws.push({ ...mapping, destination: args.slice(5), renderStatus: document.querySelector(`[data-pit-bitmap-id="${mapping.fighter}"]`)?.getAttribute("data-pit-bitmap-status") ?? null, determinant: transform.a * transform.d - transform.b * transform.c });
          }
        }
...
try {
  ...
  for (const { entry, clip } of validatedClips) {
    ...
    await page.locator('#production-facing').selectOption(clip.facing);
    ...
    for (let index = 0; index < clip.frames.length; index++) {
      ...
      const screenshot = await page.locator('canvas').screenshot({ path: path.join(output, `ahab-${clip.id}-${clip.facing}-${index}.png`) });
    ...
    checks.push({ name: 'masked-ahab-lab', atlas: entry.atlas.id, variant: masked.id, clip: clip.id, facing: clip.facing, distinctDrawings: frames.length });
  }
  ...
  checks.push({ name: 'lab-appearance-isolation', ahabAppearanceCount: appearances.length, nativeDirectionsInspected: ['right', 'left'], unmaskedAhabAtlasCount: 0, cityImportedAtlasCount: 0, historicalCityDefaultPreserved: true, mobileNoHorizontalOverflow: true });

  for (const facing of ['right', 'left']) {
    await startMatch(facing === 'right'
      ? { player: ahab.id, playerVariant: masked.id, opponent: 'city-hunter' }
      : { player: 'city-hunter', opponent: ahab.id, opponentVariant: masked.id });
    const slot = facing === 'right' ? 0 : 1, guardKey = slot === 0 ? 'KeyI' : 'Numpad9';
    for (const clip of ['idle', 'high-guard']) {
      await clearEvidence();
      if (clip === 'high-guard') await page.keyboard.down(guardKey);
      try {
        await page.waitForFunction(({ clip, facing }) => new Set(window.__v45VisualEvidence.draws
          .filter(draw => draw.clip === clip && draw.facing === facing).map(draw => draw.index)).size === 2, { clip, facing });
        const draws = (await evidence()).draws.filter(draw => draw.clip === clip);
        assert(draws.every(draw => draw.variant === masked.id && draw.facing === facing && draw.determinant > 0 && draw.destination[2] > 0 && draw.destination[3] > 0));
        const marker = page.locator(`[data-pit-bitmap-slot="${slot}"]`);
        ...
        if (clip === 'high-guard') {
          ...
          assert.deepEqual([...new Set(maintained.map(draw => draw.index))], [1], 'Guard holds its final native drawing');
        ...
        await page.locator('canvas[data-pit-arena-id]').screenshot({ path: path.join(output, `ahab-masked-${facing}-${clip}-combat.png`) });
        checks.push({ name: 'masked-ahab-live-animation', variant: masked.id, clip, facing, uniqueDrawings: 2, nativeNotMirrored: true,
          realKeyboardControl: clip === 'high-guard', ...(clip === 'high-guard' ? { heldFinalDrawing: 1, maintainedObservationMs: 550, releasedThenRestartedAtDrawing: 0 } : {}) });
      } finally { if (clip === 'high-guard') await page.keyboard.up(guardKey); }
    }
    await page.waitForTimeout(100);
    await clearEvidence();
    await page.keyboard.press(slot === 0 ? 'KeyJ' : 'Numpad1', { delay: 45 });
    await page.waitForFunction(facing => ['startup', 'active', 'recovery'].every(phase =>
      window.__v45VisualEvidence.draws.some(draw => draw.clip === `pit.stand.light.${phase}` && draw.facing === facing)), facing);
    await page.waitForFunction(facing => ['pit.stand.light.startup:0', 'pit.stand.light.active:0',
      'pit.stand.light.recovery:0', 'pit.stand.light.recovery:1'].every(key =>
        window.__v45VisualEvidence.draws.some(draw => draw.facing === facing && `${draw.clip}:${draw.index}` === key)), facing);
    const attacks = (await evidence()).draws.filter(draw => draw.clip.startsWith('pit.stand.light.'));
    assert(attacks.every(draw => draw.variant === masked.id && draw.facing === facing && draw.determinant > 0 && draw.destination[2] > 0 && draw.destination[3] > 0));
    await page.waitForTimeout(900);
    await clearEvidence();
    await page.waitForTimeout(450);
    assert.equal((await evidence()).draws.some(draw => draw.clip.startsWith('pit.stand.light.')), false, 'A released light attack cannot repeat');
    assert((await evidence()).draws.some(draw => draw.clip === 'idle' && draw.facing === facing));
    checks.push({ name: 'masked-ahab-live-light-attack', facing, actualKeyboardControl: true, phases: ['startup', 'active', 'recovery'],
      nativeNotMirrored: true, releaseReturnsIdleWithoutRepeat: true,
      observedDrawings: [...new Set(attacks.map(draw => `${draw.clip}:${draw.index}`))] });
    // Bring the actual fighters together, then block a real opponent input.
    // No combat state injection, forged event, CSS pose or replay shortcut.
    ...
    try {
      ...
      await clearEvidence();
      await page.keyboard.press(slot === 0 ? 'Numpad1' : 'KeyJ', { delay: 45 });
      await page.waitForFunction(facing => window.__v45VisualEvidence.draws.some(draw => draw.facing === facing &&
        draw.clip === 'high-guard' && draw.renderStatus === 'sprite-sheet-hold'), facing);
      const blocked = (await evidence()).draws.filter(draw => draw.renderStatus === 'sprite-sheet-hold');
      assert(blocked.length > 0);
      assert(blocked.every(draw => draw.clip === 'high-guard' && draw.index === 1 && draw.facing === facing && draw.determinant > 0));
      checks.push({ name: 'masked-ahab-blocked-impact', facing, actualOpponentKeyboardHit: true,
        retainedExactGuardPose: true, notClaimedAsImpactAnimation: true, blockedFramesObserved: blocked.length });
    } finally { await page.keyboard.up(guardKey); }
  }
  ...
  const report = { passed: true, checkedAt: new Date().toISOString(), version: 'V49', url: base, checks, errors, failures, limitations: ['Browser and keyboard validation only; no physical controller certification.', 'Ahab masked: native left/right idle, high guard and light attack. Other appearances and uncovered actions remain static source art; no complete moveset claimed.', 'The reduced roster thumbnails do not change full-resolution combat sources.'] };
