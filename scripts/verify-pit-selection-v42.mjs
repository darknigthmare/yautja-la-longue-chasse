...
const base=process.env.V42_QA_URL||'http://127.0.0.1:4174';
const output=process.env.V42_SELECTION_QA_OUTPUT||'work/v42/selection-qa';
await fs.mkdir(output,{recursive:true});
const built=await build({stdin:{contents:"export {PIT_ARCADE_LADDERS} from './app/game/systems/pitArcade.ts'; export {PIT_ARENA_IDS} from './app/game/systems/pitCombat.ts'; export {PIT_VERSUS_FIGHTER_IDS} from './app/game/systems/pitRosterExpansion.ts';",resolveDir:process.cwd()},bundle:true,platform:'node',format:'esm',write:false,logLevel:'silent'});
const {PIT_ARCADE_LADDERS,PIT_ARENA_IDS,PIT_VERSUS_FIGHTER_IDS}=await import('data:text/javascript;base64,'+Buffer.from(built.outputFiles[0].text).toString('base64'));
...
async function enter(p){await p.goto(base,{waitUntil:'networkidle',timeout:120000});await p.getByRole('button',{name:'Jouer',exact:true}).click();await p.getByRole('button',{name:'THE PIT · combat',exact:true}).click();await p.locator('[data-pit-selection-step="fighters"]').waitFor();}
...
try{
  ...
  assert((await flow.getByRole('option').count())<=24,'mounted gallery is bounded');
  ...
  await page.addInitScript(()=>{window.__selectionPad={id:'V42 virtual standard pad',index:0,connected:true,mapping:'standard',axes:[0,0,0,0],buttons:Array.from({length:17},()=>({pressed:false,touched:false,value:0}))};Object.defineProperty(navigator,'getGamepads',{configurable:true,value:()=>[window.__selectionPad,null,null,null]});});
  await enter(page);
  ...
  await page.evaluate(()=>{window.__v42SetItem=Storage.prototype.setItem;Storage.prototype.setItem=function(key,value){if(key.startsWith('yautja-long-hunt.the-pit'))throw new DOMException('V42 simulated quota failure','QuotaExceededError');return window.__v42SetItem.call(this,key,value);};});
  await confirm(page);
