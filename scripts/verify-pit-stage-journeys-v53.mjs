...
const url=process.env.V53_JOURNEYS_QA_URL||'http://127.0.0.1:4174';
const output=process.env.V53_JOURNEYS_QA_OUTPUT||'outputs/qa-commercial-audit/v53/stage-journeys/browser';
...
const routes=api.PIT_STAGE_JOURNEY_ROUTES.filter(r=>r.release==='V53');
assert.equal(routes.length,8);assert.equal(api.PIT_STAGE_JOURNEY_ROUTES.length,13);
...
try{
 for(const route of routes){
  ...
  await enterCampaignDeck(page,{url});assert.equal(await page.locator('[data-game-content-version]').first().getAttribute('data-game-content-version'),'V53');
  ...
  assert.equal(await picker.locator('option').count(),14,'Neutral plus thirteen explicit routes');assert.match(await picker.locator('..').innerText(),/Parcours disponibles · 13 liaisons d’exposition/);await picker.selectOption(route.id);
 ...
 await fs.writeFile(output+'/report.json',JSON.stringify({passed:true,checkedAt:new Date().toISOString(),url,contentVersion:'V53',checks,errors,responses,scope:'Eight original exhibition links, existing art, one confirmed throw transfer each; no canonical geography, interactive prop or complete arena claim.'},null,2)+'\n');console.log(JSON.stringify({passed:true,checks:checks.length,output}));
