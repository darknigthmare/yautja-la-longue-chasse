...
// Production identifiers do not register these humans as playable fighters.
const humanPresetByVariant = { 'machiko-clan-armor': ['machiko-noguchi'], 'marvel-2023-gold-armor-unmasked': ['theta'] };
for (const entry of uniqueHumanVariants.values()) subjects.push({
  ...
  work: entry.variantLabel, presetIds: humanPresetByVariant[entry.variantId] ?? [], flags: ['human-not-yautja'],
...
const productions = berserker ? [{ fighterId: 'berserker', file: productionPath, assets: berserker.assets }] : [];
const v34Root = 'art-source/v34/pit';
if (fs.existsSync(v34Root)) for (const folder of fs.readdirSync(v34Root, { withFileTypes: true }).filter(item => item.isDirectory())) {
  for (const name of fs.readdirSync(path.join(v34Root, folder.name)).filter(name => /provenance.*\.json$/.test(name))) {
    const file = path.join(v34Root, folder.name, name).replaceAll('\\', '/');const manifest = read(file);
    productions.push({ fighterId: manifest.fighterId ?? folder.name, file, assets: manifest.assets ?? [] });
  }
}
for (const entry of subjects) {
  entry.firstLot = families.map(family => {
    ...
    const clips = runtime.flatMap(def => def.atlas.clips.filter(clip =>
      clip.status === 'validated' && (clip.id === family.id || (family.id === 'hurt' && clip.id === 'hitstun') || clip.id.startsWith(family.id + '-') || clip.id.startsWith('pit.stand.' + family.id + '.') || (family.id === 'guard' && ['high-guard', 'low-guard'].includes(clip.id)) || (family.id === 'jump-land' && /^(jump|land)(-|$)/.test(clip.id))))
      .map(clip => ({ id: clip.id, facing: clip.facing, frameCount: clip.frames.length, atlas: def.atlas.id })));
    const authored = productions.filter(production => entry.presetIds.includes(production.fighterId) || (entry.id === 'Y-024' && production.fighterId === 'berserker'))
      .flatMap(production => production.assets.filter(asset => (asset.actionId ?? asset.id) === family.id).map(asset => ({ ...asset, productionFile: production.file })));
    const artifacts = authored.map(asset => {
      ...
      return { path: asset.sourcePath, sha256: asset.sha256, reviewStatus: asset.status, provenance: asset.productionFile };
    });
...
const allSourceReferences = subjects.flatMap(entry => entry.firstLot.flatMap(family => family.artifacts.map(asset => asset.path)));
const uniqueSources = [...new Set(allSourceReferences)];
const output = {
  schemaVersion: 1, id: 'v34-known-hunter-ten-sheet-first-lot', generator: 'scripts/build-hunter-production-v34.mjs',
  policy: {
  ...
  summary: {
    ...
    indexedSourceReferences: allSourceReferences.length,
    uniqueSourceSheetsIndexed: uniqueSources.length,
    newV34SourceSheets: uniqueSources.filter(file => file.startsWith('art-source/v34/')).length,
  },
...
const file = 'art-source/v34/known-hunters/production-manifest.json';
const serialized = JSON.stringify(output, null, 2) + '\n';
if (process.argv.includes('--check')) assert.equal(fs.readFileSync(file, 'utf8'), serialized, 'Stale V34 production inventory');
