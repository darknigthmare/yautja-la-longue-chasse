...
const requestedStage=sourceManifest.stages.find(stage=>stage.catalogueId===arenaId&&stage.number>=101&&stage.number<=136);
assert(requestedStage && requestedStage.planes.every(plane=>plane.assets.length), "V43 composition must be assembled before QA");
const conceptPreview=!extension;
assert(knownPlayableIds.includes(arenaId)||conceptPreview,"Only specified production kits may be previewed");

const output = path.resolve(process.env.V43_ARENA_QA_OUTPUT ?? "work/v43/arena-renderer-qa", arenaId);
...
const html = '<!doctype html><html lang="fr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Recette renderer THE PIT V43</title><style>body{margin:0;background:#07090c;color:#eee;font:14px system-ui}h1{font:16px system-ui;margin:6px}canvas{display:block;max-width:100%;height:auto}</style></head><body><h1>THE PIT · composition modulaire V43 · harnais de recette du renderer</h1><canvas width="960" height="540"></canvas><script src="/bundle.js"></script></body></html>';
...
try {
  ...
  assert.equal(await page.title(), "Recette renderer THE PIT V43");
  ...
  const loaded = await page.evaluate(async ({ requestedId, conceptPreview }) => {
    ...
    window.renderScenario = ({ centerX = 480, centerY = 270, zoom = 1, positions = [330, 630], reducedMotion = false, highContrast = false, frame = 0 } = {}) => {
      ...
      const drawCalls = [], originalDrawImage = context.drawImage;
      context.drawImage = function(image, ...args) {
        const src = image?.src ? new URL(image.src).pathname : '';
        if (/p0-b-vault-haze|p3-c-brazier-flame|p4-/.test(src)) drawCalls.push({src,args});
      ...
      return { drawCalls, planes: [...back.drawnPlanes, ...front.drawnPlanes], missing: [...back.missingPaths, ...front.missingPaths], fighters, unchangedState: snapshot === arenaApi.serializePitCombat(state), unchangedCamera: beforeCamera === JSON.stringify(camera) };
    };
  ...
  for (const [name, options] of [
    ...
    if (["center", "left-corner", "right-corner"].includes(name)) await page.locator("canvas").screenshot({ path: path.join(output, name + ".png") });
    scenarios.push({ name, ...report });
  }
  let ambienceProof = null;
  const ambientAssets = requestedStage.planes.flatMap(p=>p.assets).filter(a=>a.animation || a.ambientMotion);
  if (ambientAssets.length) {
    const baseline = await page.evaluate(()=>window.renderScenario({frame:0}));
    await page.locator('canvas').screenshot({path:path.join(output,'ambience-reference.png')});
    const changed = await page.evaluate(()=>window.renderScenario({frame:300+8}));
    await page.locator('canvas').screenshot({path:path.join(output,'ambience-active.png')});
    const reducedA = await page.evaluate(()=>window.renderScenario({frame:0,reducedMotion:true}));
    const reducedB = await page.evaluate(()=>window.renderScenario({frame:308,reducedMotion:true}));
    await page.locator('canvas').screenshot({path:path.join(output,'ambience-reduced.png')});
    assert.deepEqual(reducedA.drawCalls,reducedB.drawCalls,'Reduced motion must hold the real atmosphere drawings');
    assert.deepEqual(baseline.drawCalls.filter(c=>/p4-/.test(c.src)),changed.drawCalls.filter(c=>/p4-/.test(c.src)),'Atmosphere cannot move the floor');
    assert([baseline,changed,reducedA,reducedB].every(r=>r.unchangedState&&r.unchangedCamera));
    const flameLoops=[],driftingAlpha=[];
    for(const asset of ambientAssets){
      if(asset.animation){
        assert(baseline.drawCalls.some(c=>c.src===asset.frames[0].path));
        const index=Math.floor(308*asset.animation.fps/60)%asset.frames.length;
        assert(index!==0&&changed.drawCalls.some(c=>c.src===asset.frames[index].path),'Real next drawing must be selected');
        flameLoops.push({asset:asset.id,drawings:asset.frames.length,reference:asset.frames[0].path,changed:asset.frames[index].path});
      }else{
        const first=baseline.drawCalls.find(c=>c.src===asset.frames[0].path),next=changed.drawCalls.find(c=>c.src===asset.frames[0].path);
        assert(first&&next);const dx=next.args[4]-first.args[4];assert(Math.abs(dx)>1&&Math.abs(dx)<=24);
        assert.deepEqual(first.args.slice(5),next.args.slice(5),'Only the horizontal atmospheric position may change');
        driftingAlpha.push({asset:asset.id,path:first.src,measuredDeltaX:dx});
      }
    }
    ambienceProof={passed:true,flameLoops,driftingAlpha,reducedMotionHolds:true,floorUnchanged:true};
  }
  ...
  const report = { result: "PASS", arenaId, compositionDigest:arenaCompositionDigest(requestedStage), conceptPreview, playablePromotion:false, checkedAt: new Date().toISOString(), surface: "isolated-renderer-with-real-game-fighters-and-images", ambienceProof, applicationFlowVerified: false, loaded, scenarios, mobileNoOverflow: noOverflow, errors, failedRequests, screenshots:["center.png","left-corner.png","right-corner.png","mobile.png"], limit: "Controlled Chromium renderer harness. Full-app navigation, physical hardware cadence, gamepad and deployment are not certified here." };
  await fs.writeFile(`docs/v43-${arenaId}-renderer-qa.json`, JSON.stringify(report, null, 2) + "\n");
