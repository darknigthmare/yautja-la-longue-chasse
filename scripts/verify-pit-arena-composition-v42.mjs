...
const requestedStage=sourceManifest.stages.find(stage=>stage.catalogueId===arenaId&&stage.number>=21&&stage.number<=100);
assert(requestedStage && requestedStage.planes.every(plane=>plane.assets.length), "V42 composition must be assembled before QA");
...
const output = path.resolve(process.env.V42_ARENA_QA_OUTPUT ?? "work/v42/arena-renderer-qa", arenaId);
...
const html = '<!doctype html><html lang="fr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Recette renderer THE PIT V42</title><style>body{margin:0;background:#07090c;color:#eee;font:14px system-ui}h1{font:16px system-ui;margin:6px}canvas{display:block;max-width:100%;height:auto}</style></head><body><h1>THE PIT · composition modulaire V42 · harnais de recette du renderer</h1><canvas width="960" height="540"></canvas><script src="/bundle.js"></script></body></html>';
...
try {
  ...
  assert.equal(await page.title(), "Recette renderer THE PIT V42");
  ...
  await fs.writeFile(`docs/v42-${arenaId}-renderer-qa.json`, JSON.stringify(report, null, 2) + "\n");
