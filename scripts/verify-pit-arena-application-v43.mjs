...
import {parseArenaNumbers} from './lib/pit-screen-arena-v43.mjs';
const ids=parseArenaNumbers(process.env.V43_ARENA_QA_IDS??'all');
...
const output=path.resolve(process.env.V43_ARENA_APP_QA_OUTPUT??'work/v43/arena-application-qa');
await fs.mkdir(output,{recursive:true});
const manifest=JSON.parse(await fs.readFile('art-source/v33/pit-arenas/production-manifest.json','utf8'));
const selectedNumbers=ids;
...
try{
 ...
 await context.addInitScript(()=>{
  if(location.protocol.startsWith('http')&&localStorage.getItem('yautja-long-hunt.save')===null)localStorage.setItem('yautja-long-hunt.save',JSON.stringify({version:7,createdAt:'2026-09-19T12:00:00.000Z',updatedAt:'2026-09-19T12:00:00.000Z',profile:{hunterName:'Arena QA V43 isolated',rankId:'elder',honor:1900,clanMarks:83,playTimeSeconds:4321},missionProgress:{},statistics:{missionsStarted:19,missionsCompleted:17,missionsFailed:2},settings:{difficultyId:'hunter',masterVolume:0,musicVolume:0,effectsVolume:0}}));
 });
 ...
 await page.locator('[data-game-content-version="V43"]').waitFor();
 await page.getByRole('button',{name:'THE PIT · combat',exact:true}).click();
 const save=()=>page.evaluate(()=>JSON.stringify(Object.entries(localStorage).filter(([key])=>key.startsWith('yautja')).sort(([a],[b])=>a.localeCompare(b))));
 const before=await save();assert.match(before,/Arena QA V43 isolated/);
 const assertSaveUnchanged=async()=>{const after=await save();if(after===before)return;
  const previous=Object.fromEntries(JSON.parse(before)),current=Object.fromEntries(JSON.parse(after));
  report.saveMutationKeys=[...new Set([...Object.keys(previous),...Object.keys(current)])].filter(key=>previous[key]!==current[key]);
  throw new Error('Arena traversal modified storage keys: '+report.saveMutationKeys.join(', '));
 };
 await page.evaluate(()=>{window.__arenaSaveWrites=[];const original=Storage.prototype.setItem;Storage.prototype.setItem=function(k,v){if(this===localStorage&&k.startsWith('yautja'))window.__arenaSaveWrites.push(k);return original.call(this,k,v);};});
 ...
 await assertSaveUnchanged();assert.deepEqual(await page.evaluate(()=>window.__arenaSaveWrites),[]);
