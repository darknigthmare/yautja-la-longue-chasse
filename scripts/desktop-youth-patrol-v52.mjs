...
// Derived from verify-youth-patrol-v52.mjs. Only the bootstrap and captures differ:
// Electron already owns the controlled clock and needs native hidden-window capture.
...
export async function verifyDesktopYouthPatrol(page, { output, capture, checks }) {
 ...
 for (; iterations < 4500; iterations++) {
   ...
   if (s.phase !== previousPhase) { await applyKeys(new Set()); await page.clock.runFor(100); previousPhase = s.phase; await capture(s.phase + "-pc.png"); }
   ...
   if (s.phase === "patrol-ambush" && s.patrol.grazer.phase === "charge" && !resumed) {
     ...
     await capture("patrol-charge-paused-pc.png");
   ...
   if (s.phase === "patrol-defeat") {
     ...
     await capture("patrol-defeat-readable-pc.png");
 ...
 await exportStorage("patrol-return-played-storage.json"); await capture("patrol-returned-pc.png");
 ...
 await fs.writeFile(path.join(output, "patrol-keyboard-route.json"), JSON.stringify({archivePath, routeEvidence}, null, 2));
 return JSON.stringify(await saved());
}
