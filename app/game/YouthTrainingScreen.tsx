...
const ACTIVE = (phase: YouthState["phase"]) => phase.startsWith("dojo-") || phase === "camp-run" || phase === "camp-duel" || (phase.startsWith("desert-") && phase !== "desert-complete") || (isYouthPatrolPhase(phase) && !["patrol-complete", "patrol-defeat"].includes(phase)) || isYouthCagePhase(phase) && !["cage-defeat", "cage-complete"].includes(phase);
...
export default function YouthTrainingScreen(props: YouthTrainingScreenProps) {
  ...
  const lessonAction = state.phase === "dojo-jump" || state.phase === "camp-run" || state.phase === "desert-crossing" || state.phase === "patrol-ambush" ? "jump" : state.phase === "dojo-dodge" ? "dodge" : state.phase === "dojo-strike" || (state.phase === "camp-duel" || state.phase === "cage-duel") ? "light" : state.phase === "dojo-throw" ? "throw" : ["blade-award", "armory", "barracks", "desert-briefing", "desert-tracks", "desert-report", "desert-return", "patrol-briefing", "patrol-route", "patrol-assessment", "patrol-return", "cage-briefing", "cage-reward", "cage-return"].includes(state.phase) ? "interact" : null;
  ...
  useEffect(() => {
    ...
    const render = (timestamp: number) => {
      ...
      canvas.dataset.youthCage = stateRef.current.cage ? JSON.stringify(stateRef.current.cage) : "";
      canvas.dataset.youthPhaseTick = String(stateRef.current.phaseTick);
  ...
  return <section className={styles.screen} data-youth-immersive data-youth-training data-youth-step={state.phase} data-reduced-motion={reducedMotion}>
    ...
    <div className={styles.objective} data-youth-hud><h1>{objective.title}</h1><p>{objective.instruction}</p>{state.phase !== "rest" && state.phase !== "morning" && <p className={styles.shortcut}>Déplacement : {controlActionShortcut(YOUTH_CONTROL_ACTIONS.left, props.bindings)} / {controlActionShortcut(YOUTH_CONTROL_ACTIONS.right, props.bindings)}{lessonAction && <> · {lessonLabel} : {controlActionShortcut(YOUTH_CONTROL_ACTIONS[lessonAction], props.bindings)}</>}</p>}<div className={styles.measures}>{state.phase === "dojo-dodge" && state.rival.action === "jab" && state.rival.actionTick < 32 && <span role="status">{state.rival.actionTick < 12 ? "Le maître prépare son coup…" : "Esquivez maintenant !"}</span>}{objective.required > 1 && <span>Progression : {objective.counter} / {objective.required}</span>}{objective.timerTicks !== null && <span role="timer">Temps : {(objective.timerTicks / 60).toFixed(1)} s</span>}</div>
      {(state.phase === "camp-duel" || state.phase === "camp-defeat" || ["cage-duel", "cage-defeat", "cage-victory"].includes(state.phase)) && <div className={styles.duelMeters} aria-label="Équilibre du duel non létal">
        {([["vous", state.player.composure], [isYouthCagePhase(state.phase) ? "novice" : "maître", state.rival.composure]] as const).map(([name, value]) => <div className={styles.duelMeter} key={name}>
      ...
      {state.phase === "cage-duel" && state.rival.action === "jab" && state.rival.actionTick < 32 && <p className={styles.measures} role="status" data-youth-cage-telegraph>{state.rival.actionTick < 12 ? "Le novice prépare son coup…" : "Esquive ou frappe avant l’impact !"}</p>}
    ...
    <div className={styles.stage}><canvas ref={canvasRef} data-youth-stage width={960} height={540} tabIndex={0} aria-label={isYouthCagePhase(state.phase) ? "La petite Fosse jouable. Duel de novices non létal, poings, esquive et projection, sans lame. Pause pour les commandes." : isYouthPatrolPhase(state.phase) ? "Patrouille jouable de l’Unblooded. Déplacement, saut, esquive et interaction. Évitez les charges sans attaquer l’animal. Pause pour les commandes." : "Formation jouable de l’Unblooded. Déplacement, saut, esquive, frappe et projection. Pause pour les commandes."}
    ...
    {state.phase === "desert-complete" && !pending && <div className={styles.choices}><p>Le maître a reçu les trois observations et vous êtes revenu au camp. Cette sortie ne valide aucune chasse autonome. La patrouille permet ensuite de choisir le premier duel de la petite Fosse.</p><button type="button" data-youth-patrol-departure disabled={saving || effectivePaused} onClick={() => { pulse("confirm"); canvasRef.current?.focus(); }}>Poursuivre la patrouille avec le maître</button><button type="button" disabled={saving || !props.onReturnToCity} onClick={() => void returnToCity()}>Revenir dans la cité après la sortie</button></div>}
    ...
    {state.phase === "patrol-complete" && !pending && <div className={styles.choices}><p>{state.patrol && youthPatrolAssessment(state.patrol)}</p><p>Patrouille et retour enregistrés. La petite Fosse du quartier pauvre propose un duel de novices ; ce départ reste ton choix.</p><button type="button" data-youth-cage-departure disabled={saving || effectivePaused} onClick={() => { pulse("confirm"); canvasRef.current?.focus(); }}>Découvrir la petite Fosse de jeunesse</button><button type="button" disabled={saving || !props.onReturnToCity} onClick={() => void returnToCity()}>Revenir dans la cité après la patrouille</button></div>}
    {state.phase === "cage-defeat" && !effectivePaused && !pending && <div className={styles.choices} data-youth-cage-result><p>Défaite de cette tentative. Le duel est arrêté sans mise à mort ; tes preuves précédentes restent acquises.</p><button type="button" data-youth-cage-retry onClick={() => { pulse("retry"); canvasRef.current?.focus(); }}>Réessayer le duel de la Fosse</button></div>}
    {state.phase === "cage-complete" && !pending && <div className={styles.choices} data-youth-cage-result><p><strong>{YOUTH_CAGE_REWARD.name} acquis</strong> · {state.cage?.attempts} tentative(s).</p><p>{YOUTH_CAGE_REWARD.description}</p><button type="button" data-youth-cage-return disabled={saving || !props.onReturnToCity} onClick={() => void returnToCity()}>Revenir dans la cité après la Fosse</button></div>}
    {showTouch && !effectivePaused && !pending && state.phase !== "morning" && state.phase !== "desert-complete" && state.phase !== "patrol-complete" && state.phase !== "patrol-defeat" && !["cage-intro", "cage-countdown", "cage-defeat", "cage-victory", "cage-complete"].includes(state.phase) && state.phase !== "rest" && <div className={styles.touch} aria-label="Commandes tactiles de la formation">{TOUCH_ACTIONS.map(([action, label, title]) => touchButton(action, label, title))}</div>}
  </section>;
}
