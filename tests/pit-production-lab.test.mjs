...
test('production lab includes duel expansions and reports exact oriented coverage',()=>{assert.deepEqual(p.PIT_LAB_FIGHTER_IDS,p.PIT_VERSUS_FIGHTER_IDS);for(const id of p.PIT_LAB_FIGHTER_IDS.filter(id=>id.startsWith('user-')))assert.equal(p.getPitLabCoverage(id).clips,0,'static variants are never reported as animated clips');for(const id of ['tracker','greyback']){assert(p.PIT_LAB_FIGHTER_IDS.includes(id));const c=p.getPitLabCoverage(id);assert.equal(c.clips,id==='tracker'?30:28);assert.equal(c.rightClips,id==='tracker'?15:14);assert.equal(c.leftClips,id==='tracker'?15:14);assert.equal(c.sourcePages,8);}const j=p.getPitLabCoverage('jungle-hunter');assert.equal(j.clips,14);assert.equal(j.drawings,34);assert.equal(j.rightClips,6);assert.equal(j.leftClips,8);assert.equal(p.getPitLabCoverage('falconer').atlasCount,0);});
