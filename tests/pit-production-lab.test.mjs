import assert from'node:assert/strict';import test from'node:test';import{build}from'esbuild';const b=await build({stdin:{contents:"export * from './app/pit-lab/pitLabProduction';export {PIT_VERSUS_FIGHTER_IDS} from './app/game/systems/pitRosterExpansion';",resolveDir:process.cwd()},bundle:true,write:false,platform:'node',format:'esm'});const p=await import('data:text/javascript;base64,'+Buffer.from(b.outputFiles[0].text).toString('base64'));
test('production lab includes duel expansions and reports exact oriented coverage',()=>{assert.deepEqual(p.PIT_LAB_FIGHTER_IDS,p.PIT_VERSUS_FIGHTER_IDS);for(const id of p.PIT_LAB_FIGHTER_IDS.filter(id=>id.startsWith('user-')))for(const appearance of p.getPitLabAppearances(id)){const definitions=p.getPitLabDefinitions(id,appearance.id);assert(definitions.every(d=>d.variantId===appearance.id&&d.atlas.variantId===appearance.id));assert.equal(p.getPitLabCoverage(id,appearance.id).clips,definitions.flatMap(d=>d.atlas.clips.filter(c=>c.status==='validated')).length,'only the selected appearance contributes animated clips');}for(const id of ['tracker','greyback']){assert(p.PIT_LAB_FIGHTER_IDS.includes(id));const c=p.getPitLabCoverage(id);assert.equal(c.clips,id==='tracker'?30:28);assert.equal(c.rightClips,id==='tracker'?15:14);assert.equal(c.leftClips,id==='tracker'?15:14);assert.equal(c.sourcePages,8);}const j=p.getPitLabCoverage('jungle-hunter');assert.equal(j.clips,14);assert.equal(j.drawings,34);assert.equal(j.rightClips,6);assert.equal(j.leftClips,8);assert.equal(p.getPitLabCoverage('falconer').atlasCount,0);});
