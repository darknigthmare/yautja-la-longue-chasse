...
const ids = ["arena-009-quais-du-premier-sang", "arena-010-forge-des-lames-muettes", "arena-011-reserve-des-crocs", "arena-012-balcon-du-roi-de-la-chasse"];
const stages = ids.map(id => source.stages.find(stage => stage.catalogueId === id));

test("V34 concepts 9-12 own 56 original selected files with matching source receipts", async () => {
  ...
  for (const [index, stage] of stages.entries()) {
    ...
    for (const plane of stage.planes) {
      ...
      for (const asset of plane.assets) {
        ...
        assert.equal(frame.status, "reviewed");
        ...
        assert.equal(receipt.id, asset.id);
...
test("reviewed concept pictures cannot unlock an arena or increase runtime coverage", () => {
  ...
  assert.equal(summary.readyRuntimeKits, 8);
  for (const stage of stages) {
    ...
    assert.equal(api.resolvePitArenaProductionKit(stage.catalogueId), null);
