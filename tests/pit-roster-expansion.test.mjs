...
test('duel expansion never mutates twelve first-edition progression identities',()=>{assert.deepEqual(p.PIT_FIRST_EDITION_FIGHTER_IDS,first);assert.deepEqual(p.PIT_PLAYABLE_FIGHTER_IDS,first);assert.deepEqual(Object.keys(p.PIT_ARCADE_LADDERS),first);assert.deepEqual(Object.keys(p.PIT_CLAN_CIRCUITS),first);assert.deepEqual(p.PIT_EXPANSION_FIGHTER_IDS,['tracker','greyback','theta','machiko-noguchi']);assert.deepEqual(p.PIT_VERSUS_FIGHTER_IDS.filter(id=>!id.startsWith('user-')),[...first,'tracker','greyback','theta','machiko-noguchi','kok-warlord','stone-heart']);for(const id of p.PIT_EXPANSION_FIGHTER_IDS){for(const mode of ['cpu','local','training'])assert(p.canPitFighterEnterMode(id,mode));for(const mode of ['arcade','circuit','descent'])assert.equal(p.canPitFighterEnterMode(id,mode),false);assert.equal(p.PIT_ARCADE_LADDERS[id],undefined);assert.equal(p.PIT_CLAN_CIRCUITS[id],undefined);assert.equal(p.PIT_ARCADE_COSMETICS[id],undefined);assert.equal(p.PIT_EXPANSION_FIGHTERS[id].rivalId,null);assert.equal(p.PIT_EXPANSION_FIGHTERS[id].progressionAvailable,false);}assert.equal(p.isPitVersusFighterId('golden-angel'),false);assert.equal(p.isPitVersusFighterId('kok-warlord'),true);assert.equal(p.canPitFighterEnterMode('constructor','cpu'),false);});
